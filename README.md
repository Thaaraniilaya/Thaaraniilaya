<div align="center">

# Hi, I'm Thaarani Ilayaraja 👋
### AI Developer · Computer Vision & ML Engineer

<a href="https://www.linkedin.com/in/thaaraniilayaraja"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
<a href="https://thaaraniilaya.github.io/portfolio/"><img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=googlechrome&logoColor=white" /></a>
<a href="mailto:thaarani455530@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>

</div>

---

### 🧠 About Me

```yaml
role: AI Developer — Computer Vision & Machine Learning Engineer
specialization: YOLOv8 • CNN • NLP • Real-time AI Systems
experience: AI Intern @ Lavendel Consulting (2025)
  - Built a real-time gaze tracking system (MediaPipe, CNN, YOLO, DeepSORT)
education: B.Tech, AI & Data Science — Kangeyam Institute of Technology
mission: building scalable, real-time AI systems that solve real problems
```

---

### 🚀 Featured Projects

<table>
<tr>
<td width="50%">

**🌉 [BridgeGuard AI](https://github.com/Thaaraniilaya)**
YOLOv8-based bridge defect detection with automated severity analysis for infrastructure inspection.

</td>
<td width="50%">

**👀 [Real-Time Gaze Tracking System](https://github.com/Thaaraniilaya/gaze-tracker)**
CNN + MediaPipe + DeepSORT pipeline for multi-user, real-time eye gaze tracking and screen mapping.

</td>
</tr>
<tr>
<td width="50%">

**📰 Fake News Detection**
NLP-based classifier using TF-IDF and ML models to flag misinformation.

</td>
<td width="50%">

**🎧 [Chatbot Song Recommendation](https://github.com/Thaaraniilaya/Chatbot-song-recommendation)**
ML chatbot recommending music based on detected emotion.

</td>
</tr>
<tr>
<td width="50%">

**🎭 [Age, Gender & Emotion Prediction](https://github.com/Thaaraniilaya/Age-and-Gender-Prediction)**
Deep learning model for facial image analysis.

</td>
<td width="50%">

**🛠️ [KNIME Project](https://github.com/Thaaraniilaya/KNIME-PROJECT)**
Data analytics & workflow automation.

</td>
</tr>
</table>

---

### 💼 Experience

**AI Intern — Lavendel Consulting** · May 2025 – Aug 2025
Built an end-to-end real-time gaze tracking system: iris/eye landmark detection (MediaPipe), CNN-based gaze estimation, multi-user tracking (YOLO + DeepSORT), and low-latency gaze-to-screen mapping.

---

### 🧰 Tech Stack

<img src="https://skillicons.dev/icons?i=python,tensorflow,pytorch,opencv,sklearn,pandas,react,html,css,git&theme=dark" />

**Core:** Image Processing • Predictive Modeling • Business Intelligence Tools

---

### 📊 GitHub Stats

<img src="https://github-readme-stats.vercel.app/api?username=Thaaraniilaya&show_icons=true&theme=radical&hide_border=true&bg_color=0D1117" height="165" />
<img src="https://streak-stats.demolab.com/?user=Thaaraniilaya&theme=radical&hide_border=true&background=0D1117" height="165" />

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Thaaraniilaya&layout=compact&theme=radical&hide_border=true&bg_color=0D1117" height="165" />

---

<div align="center">

*"Turning data into intelligence — one model at a time."*

</div>
